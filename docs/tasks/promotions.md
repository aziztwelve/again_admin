...
Акция — это спецпредложение: при выполнении условий (нужный товар в корзине + минимальная сумма) клиент получает бесплатный подарок. Если хотя бы у одной применённой акции включены промокоды, клиент дополнительно может применить промокод вместе с подарками.
...
| Промокоды и скидки активны | checkbox | — | Если включено хотя бы у одной применённой акции — показывается поле промокода для всего набора подарков. Подарок остаётся обязательным. |
