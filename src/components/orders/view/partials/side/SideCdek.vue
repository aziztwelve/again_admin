  ...
  <section class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-900/5">
    ...
    <template v-if="isCdekDelivery">
      <dl class="mt-3 space-y-2 text-sm">
        <div><dt class="text-xs uppercase text-gray-500">Создание</dt><dd>{{ cdekOrder?.creation_state || 'Заявка ещё не создана' }}</dd></div>
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <Button size="sm" variant="outline" :disabled="loading" @click="createOrSync">
          {{ cdekOrder?.cdek_uuid ? 'Обновить статус' : 'Создать заявку' }}
        </Button>
